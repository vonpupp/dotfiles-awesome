...
local function markup(keys)
    ...
    for _, key in ipairs(keys) do
        if doc[key] then
            ...
            longest_str = math.max(longest_str, unilen(doc[key].help))
        end
    end
    ...
    for index, key in ipairs(keys) do
        if doc[key] then
            ...
            result[group] = '<span font_desc="terminus 7">' ..
            (result[group] or "") ..
            --result[group] = (result[group] or "") ..
            --'</span>' ..
            '<span foreground="Gold" font_desc="terminus 8">' ..
            string.format("%" .. (longest - unilen(skey)) .. "s  ", "") .. skey ..
            '</span>  <span color="#999999" font_desc="terminus 8">' ..
            help .. string.format("%" .. (longest_str - unilen(help)) .. "s  ", "") ..
            --'</span>'
            '</span></span>\n'
            --if not key == nil then
            --    next(index, key)
...
function display()
    ...
    for group, res in pairs(strings) do
        --if #result > 0 then result = result .. "<span font_desc='terminus 8'>\n</span>" end
        result = result ..
        "<span weight='bold' foreground='#A6E22E' font_desc='terminus 8'>" ..
        --group .. "</span>\n" .. res
        group .. "</span>\n" .. res
    end
